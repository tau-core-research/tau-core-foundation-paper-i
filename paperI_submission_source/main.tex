...
\[
  s \longmapsto \Rmap(s) \longmapsto U_i(\Rmap(s)),
\]
...
\begin{figure}[t]
  ...
  \(M_{4D}=U_{4D}(\Rmap(s))\) of a deeper parent response.  The figure is a
...
\[
\begin{array}{ll}
\textsc{toy-only} & \text{mathematically illustrative but not physical},\\
\textsc{phenomenological-only} & \text{useful fit language without parent-source derivation},\\
\textsc{blocked} & \text{known-limit, quotient, or source-realizability failure},\\
\textsc{rejected} & \text{endpoint leakage or uncontrolled rescue}.
\end{array}
\]
...
\[
  s \mapsto \Rmap(s) \mapsto U_i(\Rmap(s)).
\]
