...
\(U_{\rm cosmo}^{\Mta}(r)\) &
FLRW/LCDM limit or frozen residual channel\\
\bottomrule
\end{tabular}
...
source-overlap ledger.  As a candidate name only, this discipline may be
called residual cross-readout skeleton invariance, abbreviated \(R\)-CRSI.
The abbreviation is not a validation statistic and not an independence claim.
Without this discipline, a joint readout may simply double count the same
source coordinate.

The Weak-Signal Amplification Protocol (WSAP), developed in the Arabic-numbered
paper sequence after this foundation paper, is an operational example of this
same rule.  Its methodological slogan is: stability may suppress single-sector
amplitude, while a shared parent may preserve cross-readout coherence.  At the
foundation level this is only a protocol grammar.  It does not say that a weak
joint residual is physical evidence.  A promoted WSAP-like branch would still
need source-frozen kernels, covariance and null controls, wrong-kernel
controls, held-out prediction, and failure conditions.

The later readout-covariance and photon-lensing MAR papers sharpen the same
foundation rule from the opposite direction.  A morphology or orientation
template, for example a photon-lensing MAR statistic such as
\(T_{\tau}=B_{\rm env}A_fM_{bf}\), or its later convergence/shear and toy
Null--Matter Link extensions, may be a useful source-side handle, but it is not
by itself shared-parent evidence.  Foundation Paper I therefore treats such
templates as readout-family examples: they become physically interesting only
after the branch supplies source-freeze, null, covariance, systematics, and
failure rules.  This is why the foundation language separates the readout
family from the validation claim.
...
point: delayed-choice-looking or quantum-eraser-looking narratives need not be
read as future-to-past influence.  They can be treated, at toy level, as
different time-order readouts of one globally constrained parent record.  No
physical clock theory, retrocausality, Born rule, or quantum dynamics is
...
Subsequent Tau Core theory work has sharpened this boundary in a finite
source-strengthened toy setting.  The current finite route does not ask the
older minimal Tau grammar to force quantum mechanics.  Instead, it identifies a
source-side phase/trace skeleton: a protected primitive frame, a cyclic phase
torsor on selected pairs, reversible rank-two contrast actions, an invariant
trace/effect pairing, and one protected null policy.  When the triple loop
holonomy
\begin{equation}
  H_{ijk}=\Gamma_{ij}+\Gamma_{jk}+\Gamma_{ki}
  \quad \mathrm{mod}\;2\pi
\end{equation}
vanishes on selected triples, the pairwise phases glue to a global complex
phase chart.  In that finite and source-frozen case, the route reaches a
matrix Hilbert/Born toy packet with
\begin{equation}
  p(E|\rho)=\mathrm{Tr}(\rho E).
\end{equation}
This is a conditional finite toy recovery, not a derivation of full quantum
mechanics, collapse, tensor-product uniqueness, QFT, or empirical quantum
validation.  If this route is promoted in a later quantum paper, it should be
placed in a dedicated appendix or theorem section with all finite-domain,
frame, trace, null-policy, and source-freeze assumptions stated explicitly.
