...
\date{Draft v0.2 -- Legutóbb módosítva: 2026. szeptember 3.}
...
\textbf{Az időtlen parent realizációjának terminológiája.}
Mivel a Tau-parent időtlen, a „természet kiválasztja” fordulat nem külső
döntéshozót és nem későbbi választást jelent. Pontos tartalma a belső fizikai
implikáció
\[
 (B_\tau^{\rm phys},s_U,\mathcal L_{\rm parent})
 \Longrightarrow \mathfrak P,
\]
valamint az így létrejövő \(\mathfrak P\) csomag támogatásának fizikai
elfoglalása. Egy deklarált completionön belüli tétel csak osztályrelatív
realizációt bizonyít. Nem bizonyítja automatikusan a korlátozatlan
base--seed nyilat, és véges számú terminális kiolvasás egyezése önmagában nem
igazolja a teljes környező forrástér kimerítését. A jelen kézirat ezért a
\emph{parent-törvény általi realizáció} és a \emph{fizikai elfoglalás}
kifejezéseket használja; a régi kiválasztási nyelv csak ennek rövidítése.
\end{minipage}}
...
\begin{center}
\fbox{\begin{minipage}{0.90\linewidth}
\textbf{Observer co-descent konvenció.}
Az $O$ index kezdetben egy elfoglalt, testoldali observer-hordozó jelöltet
jelöl, nem egy leválasztott vagy már kész megfigyelőt. Az operational observer
csomagja
\[
 O_{OS}^{\rm op}
 =\bigl(B_O,\operatorname{Desc}_{OS}^{(4)},Q_{OS},E_O\bigr),
\]
ahol a lokális rank-4 Lorentz-descent reguláris, $Q_{OS}$ stabil
observer--source felbontási térkép, $E_O\neq0$ pedig stabil rekord/effektus.
Az observer és a számára hozzáférhető 4D világ ugyanannak a testkondicionált
megoldásnak a co-readoutja. Egy 4D chart vagy egy hordozótámasz önmagában nem
observer. A descent a testben lakó relációs törvény quotientje/restrikciója,
nem külön fizikai csatornaréteg és nem a parent kvantumos összeomlása.
\end{minipage}}
\end{center}

\begin{center}
\fbox{\begin{minipage}{0.90\linewidth}\small
\textbf{Az upstream reguláris 4D rekonstrukció állítási határa.}
A tipizált terminálok előtt a claim-safe preterminális lánc
\[
...
\]
Itt \(\Xi_{OS}^{\rm cont}\) a közös sima preterminális descriptor, míg a
véges operacionális rekord
\(D_{OS}^{\rm op}=Q_{OS,\delta}(\Xi_{OS}^{\rm cont})\). Az alábbi
differenciális és Hessian-állítások az előbbire vonatkoznak; a kemény
felbontási cellák az utóbbihoz tartoznak.
A
$\mathfrak C_{OS}=(\mathcal A^{\rm alg}_{OS},\mathfrak d_\tau,
R^{\mathfrak d}_{OS},\{\omega_{OS,x}\},G_{\rm occ})$ rövidítés ennek a
post-body descentnek a matematikai reprezentációja, nem új ontológiai réteg.
A $\mathfrak d_\tau$ observer-választás előtt definiált testoldali
derivációcsalád, míg
\(
 \Phi_{OS,x}=\widetilde\Phi_{OS,x}\circ R^{\mathfrak d}_{OS}
\)
az observer-látható tangentjelöltet adja. A konstans lokális rank-4,
integrálhatóság, Lorentz-konform realizálhatóság, kompatibilis sűrűség és a
meglévő ROOT/M4TR 4D hordozóval való egyezés külön nyitott kapuk. Sikerük is
csak lokális coframe-atlaszt ad; globális tetrádhoz és fermionos spin-lifthez
további topológiai feltételek kellenek. Az operational observer-csomag ezért
theorem-target, nem a téridő kész levezetése.
\end{minipage}}
\end{center}
...
\[
  ...
  (B_\tau^{\rm base},\Ssrc,\RTspace,\Rmap,\Mta,
   \{U_i\},\{N_i\},\{J_i\}).
\]
Itt \(B_\tau^{\rm base}\) a kiolvasás előtti hordozó/támasz kontextus,
\(\Ssrc\) forrásdomén, \(\RTspace\) formai válaszdomén, \(\Rmap\) térkép,
reláció vagy halmazértékű válaszszabály, \(\Mta\) pedig a stabilizált
morfológiai válaszkonfiguráció (röviden: morfológiai test). \(U_i\)
szektor-readout, \(N_i\) null/quotient politika, \(J_i\) opcionális
...
a hordozó/támasz szerepre, \(B_\tau^{\rm base}\)-re utal, nem \(\Mta\)-ra.
A seed az alapmező számára olvasható relációs/terhelési minta, ezek teljes
stabil realizált válasza pedig \(\Mta\). A „mező” szó heurisztikus: itt még
nem jelent szokásos téridődimenziót, időfejlődést, fizikai egységeket,
stressz--energiát vagy empirikus kalibrációt.

Az MRC-DEF2 a parent-oldali „objektum” jelentését is rögzíti. A base- vagy
seed-objektum egy típusos parent relációs domén objektuma, nem a readoutokból
majd visszanyerendő téridőben már eleve elhelyezkedő anyagi test. A base és a
seed pregeometrikus fizikai jelölt; korlátlan fizikai realizációjuk és
Nature-occupationjük nyitott. A részecskék, műszerek és observerek csak
\(\Mta\) tartós belső struktúráiként/readoutjaiként jelenhetnek meg downstream;
egy ilyen objektumot ugyanennek a testnek az upstream gerjesztőjeként használni
körkörös lenne.

\paragraph{PGP-CON1: pregeometrikus fizikai szerződés.}
A parent-oldalt ezért külön típusos kutatási rétegként kezeljük, nem második
szubsztanciaként vagy kész fizikai elméletként. Minimális jelölt csomagja
\[
 \mathfrak P_\tau=
...
\]
ahol a tagok a base, seed, válaszdomén, szimultán korlátok, jelölt
szimmetriák, endpoint-vak parent-törvényosztály és quotient-politika típusait
rögzítik. Az általános \(\mathcal L_{\rm pre}\) törvénynek nem kell skalár
akciónak lennie. A válaszosztály formai nemüressége, a forrás tulajdonában álló
fizikai realizáció és a nem nulla Nature-occupation három külön állítás. Csak
a már stabilizált test léphet be a későbbi observer--source descentbe; a
terminális visszanyerésből nem szabad visszafelé kiválasztani ezt a csomagot.
A teljes kanonikus szerződést és az ellenmodelleket a Tau Core theory hub
tartja fenn.
...
A minimális képződési adat
\[
 \mathfrak F_{\min}=(\mathcal Q_B,\mathsf S_B,0_B,[\mathcal A],
 \operatorname{Sel}_B,\operatorname{Stab}_B),
\]
ahol $\mathcal Q_B$ a védett testhányados, $0_B$ a fizikailag kijelölt
nulla-forrás, $[\mathcal A]$ pedig a közös forrás--test törvény.  A minimális
alap a nulla-forrású válaszstruktúra
\[
 B_{\min}=(\mathcal Q_B,[\mathcal A(0_B,\cdot)],[M_0],
 \operatorname{Sel}_B,\operatorname{Stab}_B).
\]
Ez az alap nem feltételez előzetes téridőt vagy közönséges fizikai mezőt.
A parent forrásmag az
\[
 \mathcal I_B(s;M)=\mathcal A(0_B,M)-\mathcal A(s,M)
\]
kölcsönhatás által meghatározott alaprelatív $[s]_B$ forrásosztály.  Nem
megfigyelt endpoint vagy parent-időbeli kezdeti feltétel, és nem választható ki
abból a residualból, amelyet egy későbbi kiolvasás magyarázni akar.
\end{definition}
...
\begin{definition}[Stabilizált morfológiai válaszkonfiguráció]
A Tau-oldali stabilizált morfológiai válaszkonfiguráció \(\Mta\), röviden
\emph{morfológiai test}, a kiolvasásokat kondicionáló teljes seed-kondicionált
strukturált konfiguráció. Minimálisan stabilan kiválasztott, a semleges
$[M_0]$ osztálytól különböző forrásválasz.  Nem megfigyelt 4D morfológiai
címke, nem a csupasz alapmező, és nem pusztán egy tangens válaszkoordináta.
Egy $B\oplus u_s$ lokális alakban $u_s$ csak a teljes konfiguráció egyik
koordinátája. Tartalmazhat
...
\begin{definition}[Atemporális kialakulás és tényleges bejárás]
Az alapozó nyíl
\[
 [M_\tau^\star]
 =
...
\]
forráskondicionált kiválasztást vagy stabilizációt jelent, külső parent-idő
nélkül. Egy ténylegesen bejárt $M(\lambda)\to M_\tau^\star$ út külön
occurrence/aktualitási törvényt igényel. A \(\lambda\) rendparaméter nem
observer-idő, amíg ezt külön terminális descent- és kalibrációs híd nem vezeti
le.
\end{definition}

\begin{definition}[Parent-oldali objektum és nem körkörös megjelenés]
A terminális descent előtt az „objektum” egy deklarált matematikai vagy
relációs típus objektumát jelenti. Így \(B_\tau^{\rm base}\) és \(s_U\) nem
feltételez anyagi világvonalat vagy már rendelkezésre álló téridőbeli
ütközést. Fizikai értelmezésük egy Nature által elfoglalt parent-realizációtól
függ. Egy közönséges objektumot downstream egy tartós relációs alrendszer
\(S_a\in\operatorname{Sub}_{\rm rel}(M_\tau^\star)\) és annak típusos
\(O_a=R_a(S_a)\) readoutja reprezentál. Az
\(O_a\to M_\tau^\star\to O_a\) lánc elfogadhatatlanul körkörös, hacsak egy
független, nem körkörös parent-oldali híd nincs bizonyítva.
Ez a definíció nem dönti el, hogy \(s_U\) ontológiailag független-e a
gazdagított base-től, vagy annak kitüntetett boundary-, topológiai vagy
szuperszelekciós szektora. Mindkettő nem körkörös parent-completion marad,
amíg egy fizikai forrástörvény nem választ közöttük.
\end{definition}

\begin{proposition}[Nulla-forrású faktorálás és testképződés]
Fizikailag rögzített nulla-forrás mellett
\[
 \mathcal A(s,M)=\mathcal A_B^0(M)-\mathcal I_B(s;M),\qquad
 \mathcal A_B^0(M)=\mathcal A(0_B,M)
\]
egyértelmű a testfüggetlen forráskonstansok erejéig.  Nulla-forrás nélkül egy
közös $J(M)$ tag áttehető az alap és a kölcsönhatás között, ezért a kettő nem
azonosítható külön.  Stabil nemsemleges ág esetén három külön elégséges
aktivációs út van: nemzérus első variáció, a semleges ág stabilitásvesztése,
vagy globális ágváltás.  A tétel nem bizonyítja a fizikai közös törvény, a
nulla-forrás, a szelekciós szabály vagy az elfoglalt univerzális csíra
természetbeli létezését.
\end{proposition}
...
\begin{proposition}[Egzakt örökölt-descriptor migráció]
\label{prop:mopr-migration-hu}
Rögzítsük az elfoglalt testet és observer--source kontextust, és legyen
\(\Xi_{OS}^{\rm cont}:\mathcal X_{OS}\to\mathcal Z_{OS}\) a közös folytonos
preterminális descriptor. Egy korábbi descriptor vagy termináltörvény
\(L:\mathcal X_{OS}\to\mathcal Y_L\) pontosan akkor írható egyértelműen
\begin{equation}
 L=U\circ\Xi_{OS}^{\rm cont}
 \label{eq:mopr-migration-hu}
\end{equation}
alakban az elfoglalt descriptor-képen, ha minden
\(\Xi_{OS}^{\rm cont}\)-rost egy \(L\)-rost része. A két descriptor pontosan
akkor ekvivalens, ha rostekvivalencia-relációik megegyeznek. Reguláris sima
ágon a lokális feltétel
\[
 \ker D\Xi_{OS}^{\rm cont}\subseteq\ker DL,
\]
és az egyenlőség ugyanazt a lokális quotientet adja. Az azonos rang nem elég:
\(\Xi(x,y)=x\) és \(L(x,y)=x+y\) egyaránt rank egy, de \(L\) nem faktorizál
\(\Xi\)-n keresztül.
\end{proposition}

A kemény kvantáló külön vizsgálandó:
\begin{equation}
 D_{OS}^{\rm op}=Q_{OS,\delta}\!\left(\Xi_{OS}^{\rm cont}\right).
 \label{eq:operational-record-hu}
\end{equation}
A fenti tétel egy már megadott termináltörvényt vagy kalibrációt visz át;
nem vezeti le a fizikai \(Q_{OS,\delta}\)-t, kalibrációt, a galaktikus
\(q_R(R)\)-t vagy a természeti elfoglalást. A kemény felbontási cellák csak
akkor öröklődnek, ha a megfelelő stabil partíciók is kompatibilisek.
...
A morfológiai test kötelező része szándékosan kicsi: meg kell adnia a
forrásdomént, a readout-családot, a null-politikát és az adott ág
admisszibilitási/fagyasztási szabályát.  Szektor-felbontás, falak,
Hessian-támasz, history és observer/path szerkezet csak akkor kötelező, ha az
ág readout-szabálya ténylegesen használja őket.  Így egy megfigyelt galaxis
morfológiája, egy kvantum hozzáférési struktúra és egy observer-time struktúra
nem ugyanaz a morfológiafajta, hanem a \(\Mta\) különböző alrekordjainak
vetített handle-je lehet.
...
megfigyelő ideje a vele együtt stabilizálódó observer-readout csomagjához
tartozik; nem egy előre létező observerre utólag ráhelyezett torzítás. Egy későbbi
fizikai elméletnek kell megmutatnia, mikor redukálódik sajátidőre és mikor
enged korrekciókat.

Ennek pontos korlátja van. Egy koherens, forrásrend szerint felépített részleges
testcsalád és annak teljes kolimitje belsőleg megfigyelésileg ekvivalens, ha
minden megengedett terminális readout a teljes test megfigyelő számára
hozzáférhető restrikcióján keresztül faktorizál. Ezért a konzervatív elsődleges
értelmezés az atemporálisan teljes test valós oksági orientációval és
observer-relatív kibontással; ez azonban nem bizonyítja, hogy a szó szerinti,
lépésenkénti ontikus morfogenezis hamis. A kettő megkülönböztetéséhez olyan
forrásból levezetett és terminálisan látható stádium-mennyiség kellene, amely
nem rekonstruálható a teljes testből és a hozzáférhető tartományból. Jelenleg
nincs ilyen mennyiség. A metrikus külső parent-idő ezért nem a
\eqref{eq:observer-time-hu} összetevője, hanem egy nem kiválasztott többletmodell.

A későbbi temporális elemzés ezt úgy pontosítja, hogy a temporális ág nem
rivális fundamentális ontológia, hanem a temporalizálható atemporális hordozók
fölötti integrálható rendstruktúrák tere. Véges összefüggő relációs komplexen
akkor létezik egyetlen skalár proto-idő, ha az előírt előfordulási növekmények
minden előjeles körösszege eltűnik a fizikai nullok leválasztása után. Az
enriched előfordulás--hatás modellben
$\omega_{\rm occ}=-d\mathcal A_{\rm occ}$ és az EOCC feltételesen pozitív
aktualitási tanúsítványt ad az egyensúlytól távol. Ez nem bizonyítja, hogy a
korlátlan fizikai parent vagy a természet ezt a komplementumot realizálja.

A későbbi forrásaudit a négyes rangú átadást is pontosítja. A megtartott CCOC
összekötő csak a háromdimenziós relatív observer--source szektoron hat, míg az
OGAS a kontakt-sort és az előfordulási sort egy közös, előjeles lokális
négyes rangú szelvényre helyezi. Ebből még nem következik teljes leképezés a
$T(\mathbb{CP}^{1}_{s}\times\mathbb{CP}^{1}_{b})$ projektív érintőtér és az
$E_T\oplus E_S$ testfelbontás között, sem a két térfogatmérték azonossága. Ha
egy ilyen, forrás által birtokolt, felbontást és mértéket megőrző összekötőt
külön feltételezünk, akkor a közös irány Jacobi-tényezője egyértelműen
$g_{FFL}^{-3}$; ezen a feltételes szinten nincs új illeszthető idő-gain. Az
összekötő fizikai létezése és elfoglalása továbbra is nyitott.

Az M4 hossz--idő--akció kalibráció ettől külön kérdés. A közös kúp-,
gravitáció--akció- és akcióperiódus-azonosságok a fennmaradó szabadságot az
egyetlen pozitív
\[
 (L_H,T_H,\mathcal A_*)
 \longmapsto
 ...
 \qquad \sigma>0
\]
metrológiai pályára redukálják. Ez megőrzi a fénykúpot, az oksági rendet, a
sajátidőarányokat és minden dimenziótlan óra--vonalzó megfigyelhetőt. Egy
mért, dimenzióhordozó referencia kiválaszt egy SI-reprezentánst, de a
normalizált morfológia nem jósolja meg annak decimális SI-értékét, és ebből a
dimenziótlan csatolások sem következnek.

Az ontológiai korlát ennél erősebb: a jelenlegi temporális osztály minden
véges, aciklikus, forrásfagyasztott lokális folyamata --- az előre rögzített és
adaptív beavatkozásokkal, sztochasztikus kernelekkel és közös külső zajjal
csatolt kontrafaktuális másolatokkal együtt --- operacionálisan azonos
atemporális, szimultán faktorgráfként is felírható. Ez nem az atemporalitás
bizonyítása, hanem annak tétele, hogy ezek a véges belső kísérletek önmagukban
nem bizonyítják a szó szerinti parent-bejárást.
...
Az optikai specializáció ezt tovább pontosítja. A jelenlegi korpusz két
illesztésmentes transzportmechanizmust tartalmaz: a teljes testkapcsolat
holonómiáját, illetve egy változó, forrásbirtokolt hozzáférési projektor
Kato-transzportját. Egy pusztán skaláris $U(1)$ fázis nem határozza meg egy
többmódusú normalizált Choi-folyamat nemskaláris projektív orientációját,
miközben teljes, állandó hozzáférésnél $P_O=I_n$, ezért
$[dP_O,P_O]=0$. Ez nem Tau-hatás hiányát bizonyítja, hanem azt, hogy a
nemtriviális optikai transzporthoz a teljes testkapcsolatot vagy a ténylegesen
változó hozzáférési alteret függetlenül kell forrásolni; a terminális
átviteli görbéből ezeket nem szabad visszafelé definiálni.
...
A következő foundation-szintű mérföldkőnek szűkebbnek kell lennie, mint egy
újabb általános framework-réteg.  Egyetlen worked branch a megfelelő cél:
observer-time, finite readout-time, vagy ehhez hasonlóan kicsi ág.  Egy ilyen
ágnak választania kell egy konkrét matematikai típust \(\Rmap\)-ra, meg kell
indokolnia, hogy ez a típus miért admisszibilis az adott ághoz, vissza kell
adnia legalább egy ismert limitet, és endpoint-inspekció előtt fagyasztania
kell legalább egy kizárási szabályt.  A cél nem az, hogy egyszerre oldja meg a
sötét anyagot, kvantummechanikát, általános relativitást és kozmológiát, hanem
az, hogy egy minimális esetben megmutassa: a readout-csomag fegyelmezett
kalkulusként is működhet.
...
\begin{itemize}
  ...
  \item \textbf{Foundation Paper II}: véges source-signature, GNS-egyediség,
  marginális nem-azonosíthatóság és közös incidence.
  \item \textbf{Foundation Paper III}: complete record edge-ek, út-holonómia,
  irreverzibilis CP-mapek és recovery-correctable readoutok.
\end{itemize}

A Paper II új közösforrás-kompressziós tétele szerint egy koherens
\(\mathfrak J_*=(\Pi_*,\varphi_*)\) reprezentáció--állapot adat egyszerre
rögzíti a kevert relációideált, a separator-multiplicitást, a relatív
\(P3\)--\(P4\) ragasztást és a megtartott blokkok hű foglaltságát. Ezek tehát
az enriched MVP-ben nem független kapcsolók. A jelenlegi szűk forrás azonban
nem választja ki \(\mathfrak J_*\)-ot; a korlátlan parent-törvényi realizáció
és a globális terminálcsalád teljessége nyitott marad.
...
\subsection{A véges closure konkrét pointed-exterior realizációja}

A véges lokális closure-nak most van egy explicit közös-forrású realizációja.
Egy teljes rangú, háromdimenziós orientált seed-incidencia a
\[
 \Lambda^2J_s\oplus\Lambda^3J_s
\]
exterior csomagot indukálja, amelynek rangja $3+1$.  A két fok ugyanazt a
Hodge-metrikát és ugyanazt az akcióegységet örökli.  Pointed-exterior csíra,
hűséges Hodge-alapmező és source-fidelity/no-bypass törvény mellett mindkét fok
fizikailag elfoglalt a deklarált gazdagított parent-osztályban.

Ez nem korlátlan Nature-tétel.  A szűk reláció/rendezés seed-reduktum nem
kényszeríti ki a teljes exterior állapot- és lehetőséggrammatikát, és a meglévő
top-form closure reprezentációelméleti okból nem állítja elő lineárisan az
axiális páros closure-t.  A részletes bizonyítást a Technical Paper VII-B
birtokolja; a Paper I csak az architekturális következményt örökli.

\subsection{Közös kvantum--gravitációs transzport és akcióegység}

Legyen egy hű, elfoglalt közös forráshányados pozitív generátora $K_*$,
kvantum- és gravitációs terminálképe pedig $W_Q$, illetve $W_G$.  Az elfoglalt
képeken az egyetlen forráskompatibilis transzport
\[
 T_{G\leftarrow Q}=W_GW_Q^{-1},\qquad
 T_{G\leftarrow Q}K_Q=K_GT_{G\leftarrow Q}.
\]
Ha mindkét terminál metrikája ugyanarra a source-Hessianra húzódik vissza,
akkor $T_{G\leftarrow Q}^\dagger h_GT_{G\leftarrow Q}=h_Q$.  A két readout
közötti skálázás tehát nem szabad illesztési paraméter, miután a fizikai
forrástérképek rögzítettek.

A deklarált SHR--PSAL--QPHASE common-action completionben a standard kvantumos
fáziskalibráció
\[
...
\]
azonosságot adja, így $\ell_H^2=48\pi\hbar G/c^3$ a használt
akciókonvencióban.  Ez feltételes közös normalizációs zár, nem $\hbar$ SI
értékének parentből való jóslata és nem Nature-szintű kiválasztás.  Külön
publikált kvantum- és gravitációs marginálisokból a közös forrástérképek és a
közös kovariancia nem rekonstruálhatók.

A fennmaradó fanout-szabadság is lezárul egy szűkebb gazdagított csomagon
belül.  Ha az elfoglalt közös forrásszektor abszolút irreducibilis, kvantum-
és gravitációs kópiái multiplicitás-egy típusúak, a termináltérképek pedig
forrásekvivariánsak, akkor a Schur-merevség minden nem nulla térképet skalárig
egyedivé tesz.  A ciklikus no-bypass elfoglaltság kizárja a nulltérképet, a
közös visszahúzott metrika rögzíti a skalár nagyságát, egy forrástulajdonú
orientált horgony pedig a jelét.  Így a fenti transzport az egyetlen
forrástermészetes izometria.  A nulltérkép, az ismételt kópia és a rögzítetlen
horgony ellenmodellje megmutatja, hogy minden premissza szükséges.  Ez belső,
feltételes lezárás; nem bizonyítja, hogy a korlátozatlan fizikai base--seed
törvény ezt a szektort foglalja el.

Egy endpoint-vak, Schur-semleges relációdefektus-négyzet bármely megadott
pozitív válasz-Hessiant és vegyes source-jetet előállíthat úgy, hogy a post-body
változó minimalizálása után pontosan az eredeti seed--body akció marad.  Ez a
skaláris akcióalak keresését visszahatás nélkül lezárja.  Ortogonálisan
elforgatott terminálprojektorok azonban változatlanul hagyják ezt az akciót,
miközben megváltoztatják a megnevezett terminálválaszokat.  A deklarált
osztály minimális elegendő objektuma ezért a joint jetet és a forrástulajdonú
szerepreprezentációt együtt tartalmazó teljes grádált forráskarakterisztika.
Ennek fizikai tulajdonjoga továbbra is nyitott.

A pointed exterior eredmény tovább szűkíti ezt a fanout-csomagot. Egy onto,
no-bypass CAR-pályán az üres gyök ciklikus, a reprezentált forrásalgebra teljes
mátrixalgebra, fizikai modulja abszolút irreducibilis és nincs spectator
kópiája. Ha ezt egy forrástulajdonú, orientált kétszerepű síkkal tenzorozzuk,
pontosan egy kvantum- és egy gravitációs kópiát kapunk; a közös szorzatmetrika
ezután megadja a fenti Schur-zárás többi feltételét. Ilyen típusozott szerepsík
nélkül viszont egy $U(2)$-forgatás változatlanul hagyja a típusozatlan skalár
akciót és metrikát, miközben megváltoztatja a megnevezett projektorokat; a
null-láb és duplikált-kópia kontrollok is fennmaradnak. A nyitott fizikai input
tehát egyetlen nem nulla, orientált Q/G incidencia a teljes grádált
forrásreprezentációban, nem hat független fanout-együttható. A deklarált
gazdagított P3 és Tau-lokális kompozíciós osztályban ez az incidencia nem új
connector: a két forrástulajdonú, nem nulla rendezett kotangens port metrikus
duálisa adja a Q/G-vonalakat, a lokális forráskópia-faktorizáció pedig
szerepenként egy kópiát. Egy hidden-diagonal kontroll teljes Q- és G-marginális
rang mellett megfelezi a joint rangot, ezért a faktorizációs feltevés
lényeges. A paper nem vezeti le, hogy a fizikai base--seed törvény a
gazdagított portcsomagot és ezt a Tau-lokális faktorizációt együtt realizálja.

A két fennmaradó premisszát a korábban megkonstruált komponenslokális
gazdagított akció,
$\widehat\Gamma_{\rm CLP3}=\sum_C\widehat\Gamma_C$, együtt realizálja. A
nemnegatív, egyszer számolt komponensakciók minimumai elérhetők, ezért minden
globális minimizáló egyszerre foglalja el a P3 portcsomagot és az LC1--LC4
lokalitást. A pointed-CAR és Schur tételekkel kompozálva ebből az egyedi
normalizált izometrikus Q/G-transzport, a faithful fáziságon pedig
$\mathcal A_*=\hbar$ következik új connector és gain nélkül. Ez lezárja a
feltételes completion-to-fanout láncot. Azt nem bizonyítja, hogy a természet a
$\Gamma_{\rm CLP3}$ törvényt választja: hidden-global-relation és hiányzó-port
completionök ugyanazt a szűk upstream reduktot adják, mégis megsértik a
következtetést.

Pontosabban a fizikai cél nem egy kitüntetett off-shell skalárképlet. A fanout
invariáns azon nemnegatív completionök osztályán, amelyeknek ugyanaz az
elfoglalt minimumhalmaza, a közös egységben felírt valódi-normális fizikai
Hessianja és a tipizált port/root/fázis csomagja. Valóban,
$\Gamma_\lambda=\Gamma_{\rm CLP3}+\lambda\Gamma_{\rm CLP3}^2$ minden
$\lambda>0$ esetén ugyanazt a minimumot, első variációt és fizikai Hessiánt
adja, miközben off shell eltér. A természeti kiválasztás ezért ebbe a
completion-osztályba tartozást jelent; a rejtett globális és hiányzó-port
kontrollok az osztályon kívül vannak.

\subsection*{A lokális fizikai hányados örökölt akcionális zárása}

A IV. alapozó paper ACT-PCLOSE1 tétele most pontos feltételes hidat ad egy
rendezett Parent-akciócsomagtól az összes deklarált readoutig. A csíra--test
megoldás kiválasztása és befagyasztása után a reguláris post-body stacionaritás
eredménye
\[
 D_SZ_*=-H_Z^{-1}B_{ZS},
 \qquad O_i=Q_{OS,i}\circ T_i\circ P_i\circ\Xi(Z_*).
\]
A halmozott terminálok pontosan akkor rekonstruálják a deklarált lokális
fizikai hányadost, ha
$\ker J_{\rm term}=\mathcal N_{\rm phys}$. Ez azt jelenti, hogy az idő-,
kvantum-, gravitációs és más kimenetek egy közös válasz tipizált leszármazottai,
nem egymástól független illesztések. A tétel nem bizonyít globális
forrásteljességet, természeti elfoglalást, Standard Modell-tartalmat, abszolút
mértékegységeket vagy nem nulla Tau-specifikus jelet. Egy stabil,
terminálvak forrásszektor pontos ellenpélda minden erősebb következtetésre.

Az aktív post-body solderhez nem kell új, illesztett mátrix, ha ugyanennek a
befagyasztott akciónak a kevert Hessianjából olvassuk ki. A válasz-Hessian
\(H\) és a kevert Hessian \(B\) mellett az implicit stacionaritás
\begin{equation}
 \Gamma=D_YZ_*=-H^{-1}B.
 \label{eq:active-solder-mixed-hessian-hu}
\end{equation}
Az örökölt SBTCL106 Bregman-akciónál
\(H=G_{OS}\), \(B=-G_{OS}J_{OS}\), ezért
\(\Gamma=J_{OS}\) és \(C-B^*H^{-1}B=0\). Ez belső, feltételes redukció,
nem fizikai forráskiválasztás: az aktív forrásváltozón keresztüli descenthez
továbbra is kell
\(p_sJ_{OS}|_{\ker DF}=0\), amelynek forráseredetét és nem nulla fizikai
elfoglalását a pozitivitás vagy a Schur-semlegesség nem bizonyítja.

\subsection*{Általános belső megfigyelő--cél visszahatás}

A megfigyelő a közös Parent/readout konfiguráció része, ezért a mérés
átrendezheti a közös post-body állapotot, de ebből nem következik a már
stabilizált \(M_\tau^\star\) újbóli kiválasztása. Rögzített test és egy
megadott reguláris \(\mathcal A_{\rm post}^{M_*}(x,o)\) funkcionál mellett
\[
 D_ox_*=-H_X^{-1}C_{XO},\qquad
 H_X=D_x^2\mathcal A_{\rm post}^{M_*},\quad
 C_{XO}=D_xD_o\mathcal A_{\rm post}^{M_*}.
\]
Egy sima terminális előkép változása közvetlen megfigyelői
kontextushatásra és a célon közvetített visszahatásra bomlik. Kemény
kvantálásnál ezt azonos kontextusú, rögzített-cél kontrollal kell szétválasztani.
Terminál-null vagy azonos readout-cella elrejthet valós visszahatást, míg
puszta kontextusváltozás nulla \(C_{XO}\) mellett is adhat terminális eltérést.
Ez feltételes matematikai tétel, nem a fizikai \(C_{XO}\), a Nature-occupation
vagy egy Tau-specifikus jel bizonyítása.
...
\begin{itemize}
  ...
  \item Nem vezeti le a fizikai \(Q_{OS,\delta}\)-t, a terminális
  kalibrációt vagy a galaktikus \(q_R(R)\)-t.
...
\section{Feltételes gráf/Hilbert-reprezentáció és az oksági irány}

Komplex kompatibilitási gráf csak akkor vezethető be, miután a forrás, a
morfológiai test és az observer--forrás descent már meghatározott egy
$\mathcal A_{OS}$ algebrát, egy pozitív $\omega_{OS}$ állapotot és nemnulla
$x_a$ próbaelemeket. Ekkor a normalizált GNS-átfedések
\begin{equation}
 G^{OS}_{ab}=
 \frac{\omega_{OS}(x_a^*x_b)}
 {\sqrt{\omega_{OS}(x_a^*x_a)\omega_{OS}(x_b^*x_b)}}
\end{equation}
Hermitikus, pozitív szemidefinit, egységnyi diagonálú Gram-mátrixot adnak.
Megfordítva minden ilyen véges mátrix reprezentálható egységvektorok
Gram-mátrixaként, minimálisan $\operatorname{rank}G^{OS}$ dimenzióban. Ez
standard matematika, nem új Parent-kiválasztási tétel.

A megengedett irány ezért
\[
 B_\tau^{\rm base}+\sigma_U
 \longrightarrow M_\tau
 \longrightarrow (\mathcal A_{OS},\omega_{OS},\{x_a\})
 \longrightarrow G^{OS}
 \longrightarrow \hbox{típusos terminál-readoutok}.
\]
A fordított következtetés nem érvényes. Az $I_n$ és a
$\mathbf1\mathbf1^*$ egyaránt egységdiagonálú pozitív Gram-mátrix, mégis
teljesen eltérő rangot és korrelációs szerkezetet hordoz. A Gram-pozitivitás
tehát nem választja ki a csírát, nem stabilizálja a testet, nem azonosítja az
observert és nem bizonyít fizikai elfoglaltságot. Az alacsony spektrális
módusok, projektív fázisok és gráfklaszterek csak feltételes readout-
diagnosztikák maradnak, amíg egy forráseredetű akció és termináltérkép nem
ad nekik fizikai szerepet. A Hilbert-gráf így egy már levezetett közös
readout reprezentációja lehet, nem második Parent-ontológia.
...
Az atemporális--temporális viszony ezért réteges: az atemporális hordozó a
tágabb kinematikai alap, a temporális komplementum pedig integrálható
rend/aktualitási szerkezetet ad egy temporalizálható hordozóhoz. Az EOCC ezt
egy deklarált enriched forrásosztályban aktualizálja, de a véges intervenciós
szimulációs no-go megakadályozza, hogy az operacionális sikerből szó szerinti
Parent-evolúciót következtessünk.
...
\section{A megfigyel\H{o} fizikai megval\'{o}s\'{\i}t\'{a}s\'{a}nak hat\'{a}ra}
Az \'{u}j eredm\'{e}nyek felt\'{e}teles matematikai levezet\'{e}sek.
A k\"{o}z\"{o}s csatolt t\"{o}rv\'{e}ny meghat\'{a}rozhatja a helyi
kvantumos bizonytalans\'{a}got, de nem azonos\'{\i}tja a term\'{e}szetben
megval\'{o}sult megfigyel\H{o}t. Az alap\'{a}llapot kisz\'{a}m\'{\i}t\'{a}sa
nem el\H{o}k\'{e}sz\'{\i}t\'{e}si folyamat; a kovariancia nem stabil
felbont\'{a}si t\"{o}rv\'{e}ny. A fizikai kontextus, energia- \'{e}s
id\H{o}kalibr\'{a}ci\'{o}, el\H{o}k\'{e}sz\'{\i}t\'{e}s \'{e}s
$Q_{OS}$ tov\'{a}bbra is nyitott. A r\'{e}szletes bizony\'{\i}t\'{a}sok
az angol kvantumos paperben \'{e}s a body-readout matematikai companionban
szerepelnek; empirikus eredm\'{e}nyt ez a friss\'{\i}t\'{e}s nem m\'{o}dos\'{\i}t.
% END OBSERVER UPDATE 20260914

% BEGIN LAB UPDATE 20260915
\section{A numerikus cs\'{\i}rakeres\'{e}s hat\'{o}k\"{o}re}
A lab v\'{e}ges, megadott forr\'{a}smodellje stabiliz\'{a}lt testet,
majd k\"{u}l\"{o}n felt\'{e}telezett observer-v\'{a}laszt sz\'{a}m\'{\i}t.
A modul\'{a}ris gr\'{a}f kompatibilit\'{a}sa konstrukci\'{o}s feltev\'{e}tel.
A numerikus rangsor nem fizikai univerzum-\'{e}letk\'{e}pess\'{e}g,
a gener\'{a}ci\'{o}sz\'{a}m nem Parent-id\H{o}. A forr\'{a}s fizikai
kiv\'{a}laszt\'{a}sa, a kalibr\'{a}ci\'{o} \'{e}s a megfigyelt
univerzumhoz val\'{o} f\"{u}ggetlen illeszked\'{e}s nyitott marad.
% END LAB UPDATE 20260915

% BEGIN STATE UPDATE 20260918
\section{A fizikai forr\'{a}sazonos\'{\i}t\'{a}s jelenlegi hat\'{a}ra}
A teljes observer--forr\'{a}s csomag felt\'{e}telesen meghat\'{a}rozza a
m\'{e}r\'{e}si val\'{o}sz\'{\i}n\H{u}s\'{e}geket. Azonos statisztika
azonban nem jelent azonos m\'{e}r\'{e}s ut\'{a}ni \'{a}llapotot. A v\'{e}ges
kvantumos modellben a k\"{o}z\"{o}s korrel\'{a}ci\'{o}khoz val\'{o}
hozz\'{a}f\'{e}r\'{e}s helyileg l\'{a}thatatlan k\"{u}l\"{o}nbs\'{e}get
tehet kiolvashat\'{o}v\'{a}. Ez standard kvantummechanik\'{a}val kompatibilis,
nem Tau-specifikus jel. A fizikai csatol\'{a}s, el\H{o}k\'{e}sz\'{\i}t\'{e}s,
kalibr\'{a}ci\'{o} \'{e}s stabil $Q_{OS}$ kiv\'{a}laszt\'{a}sa nyitott.
A tempor\'{a}lis \'{a}gban az eredeti gradiens-EOCC, az induk\'{a}lt
ellen\'{a}ll\'{a}s k\"{o}zel\'{\i}t\'{e}se \'{e}s a tehetetlens\'{e}gi
kib\H{o}v\'{\i}t\'{e}s k\"{u}l\"{o}n t\"{o}rv\'{e}ny. Azonos egyens\'{u}ly
nem bizony\'{\i}t azonos fejl\H{o}d\'{e}st. Az empirikus pontsz\'{a}mok
v\'{a}ltozatlanok. A r\'{e}szletes levezet\'{e}sek a VI., V. paperben \'{e}s
a body-readout, illetve seed-body matematikai companionban szerepelnek.
