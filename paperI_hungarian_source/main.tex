...
\title{\textbf{Tau Core: időtlen morfológiai kiolvasási keret az emergens fizikai dinamikához}\\
...
egy mélyebb, időtlen válaszszerkezet effektív kiolvasása.  A minimális formai
gerinc:
\[
  s \longmapsto \Rtau(s) \longmapsto U_i(\Rtau(s)),
\]
ahol \(s\) parent-oldali forrásmag, \(\Rtau(s)\) endpoint-vak Tau-válasz, és
\(U_i\) szektor-kiolvasás: például téridő-, megfigyelői idő-, kvantum-,
gravitációs vagy tömeg/forrás-kiolvasás.  A központi állítás architekturális:
a formalizmus nyelvet ad a definíciók, posztulátumok, feltételes
következmények, toy mechanizmusok és későbbi empirikus tesztek
szétválasztására.  A kézirat elhatárolja a keretet a blokk-univerzum
ontológiától és a szokásos téridő-mezőelmélettől, definiálja a Tau-oldali
morfológiai testet, hét alapozó posztulátumot fogalmaz meg, a relift-kudarc
nyelvén bevezeti a kiolvasás-relatív rejtett defektust, és véges toy
modelleket ad a rejtettségre és az effektív időre.  A fő blokkolók is
explicit módon szerepelnek: a paper nem vezeti le az általános
relativitás\-elméletet, a kvantummechanikát, a Born-szabályt, a kvantum\-térelméletet,
a Standard Modellt, a sötét anyagot, a sötét energiát vagy egy fizikai parent
akciót.  Ez pozíció-paper és formai sejtésváz, nem empirikus bizonyíték új
fizikai törvényre.
\end{abstract}
...
\begin{figure}[t]
  ...
  parent-oldali forrásmag \(s\) endpoint-vak Tau-választ \(\Rtau(s)\) indukál.
...
Formai gerinc & Minimális kalkulus, forrásfegyelem, effektív idő, posztulátumok & Keretposztulátumok\\
...
\(\Rtau(s)\) & endpoint-vak válasz & időfejlődés\\
...
harmadik pedig effektív dinamikává fordít.
...
szerkezet.  Minden kiolvasásnak saját null-iránya és validációs határa van.
\end{definition}
...
\begin{definition}[Kiolvasási defektus]
Ha \(r=\Rtau(s)\), akkor a kiolvasási defektus
\[
  h_i(r)=J_i(U_i(r))-r,
  \label{eq:defect-hu}
\]
amikor a kivonás értelmezett.  Általánosabban: annak ekvivalenciaosztálya,
hogy \(J_iU_i(r)\) nem tér vissza az eredeti parent-válasz reprezentánshoz.
\end{definition}

Az \eqref{eq:defect-hu} egyenlet nem sötétanyag-képlet és nem kvantum-kollapszus
képlet.  Absztrakt hiányossági rekord.  Egy fizikai ágnak később meg kell
adnia a kodomént, a quotientet, az egységeket, az ismert limiteket és az
empirikus tesztet.

\section{Minimális kiolvasási kalkulus}

Legyen \(\mathcal R_{\tau}\) parent-választér és \(U_i\) kiolvasás.  Az első
...
\(\mathcal R_{\tau}\) lineáris és \(U_i\) lineáris, ez a szokásos nulltérre
...
Ezért a több-kiolvasásos tesztek értékesek lehetnek, de veszélyesek is: ha két
kiolvasás nem független, akkor ugyanazt a forráskoordinátát kétszer számolhatja.
Érett Tau Core ágnak ezért source-overlap ledgerre van szüksége.
...
\end{equation}
Ha ez sérül, a readout különböző megfigyelhetőket rendel olyan parent
...
\section{Hogyan jelenik meg effektív dinamika?}

A Tau Core a primitív időfejlődést readout-paraméterezett változással váltja
ki.  Legyen \(r=\Rtau(s)\) parent-szinten időtlen.  Tegyük fel, hogy egy
...
Ez a minimális értelem, amelyben a dinamika emergens.
...
\section{Toy modell effektív idővel}
...
\]
A \(k\) index effektív időkoordinátaként választható, és véges differencia
írható:
\[
...
\begin{figure}[t]
  ...
  \caption{Véges toy modell effektív időre.  A parent rekord maga nem fejlődik.
...
\(\Rtau(s)\), illetve bármely source objektum, amely readout-kernelt választ,
...
\begin{postulate}[Effektív dinamika]
Időfejlődés csak akkor jelenik meg, ha egy readout stabil rendezést vagy
óra-koordinátát enged meg.  Egy \(\frac{d}{dt}U_i(\Rtau(s))\) derivált
effektív readout-derivált, nem primitív parent-idő derivált.
\end{postulate}

\begin{postulate}[Rejtett defektus]
Ha egy readout és megengedett reliftje nem állítja vissza a parent-válasz
reprezentánst a deklarált null-reláció modulo, a nem reliftelhető komponens a
readout rejtett defektusaként kerül rögzítésre.
\end{postulate}
...
\begin{figure}[t]
  ...
  ad effektív időre és rejtett defektusokra.  Nem validál empirikusan, és nem
...
\[
  \mathcal R_{\tau}=\mathbb R^3,\qquad r=(a,b,c).
\]
Két readout:
\[
...
\end{equation}
A szögletes zárójel azt jelzi, hogy a readout-térkép függhet a morfológiai
testtől.  Ez nem bizonyítja, hogy \(\Mta\) fizikailag le van vezetve; csak azt
mondja, hogy a readout-választás nem forrásmentes.
...
\begin{equation}
  h_g=J_gU_g(\Rtau(s))-\Rtau(s).
\end{equation}
Ha \(h_g\) egy komponense túléli a deklarált null quotientet és mérhető 4D
readoutja van, akkor residual-csatorna jelölt.  Ez még nem sötétanyag-helyettesítés.
...
\begin{enumerate}[label=(\arabic*)]
  \item nem körkörös parent válaszlaw \(\Rtau\) levezetése;
...
\[
\begin{array}{ll}
\text{Paper 1--14} & \text{korlátos empirikus vagy protokoll-csomagok},\\
...
\]
Itt \(\Ssrc\) forrásdomén, \(\mathcal R_{\tau}\) válaszdomén, \(\Rtau\)
...
\begin{align}
  ...
  &r=\Rtau(s) \text{ definiált a deklarált forrásdoménen},\\
  &rN_i r'\Rightarrow U_i(r)=U_i(r'),\\
  &h_i(r)=J_iU_i(r)-r \text{ csak } N_i \text{ modulo értelmezett},\\
